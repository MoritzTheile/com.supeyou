package com.hotelorga.core.web.client.rpc.guest2room.assoaeditor.item;
...
public class ItemWidget extends WidgetView {
	...
	private void render() {
		...
		if (displayStart != null && displayEnd != null && thisDTO.getFromDate() != null && thisDTO.getToDate() != null) {
	...
	private String renderLabel(Guest2RoomDTO guest2roomDTO) {
		...
		description += guest2roomDTO.getDtoA().getFirstname();
		description += " " + guest2roomDTO.getDtoA().getLastname();
		description += ", geb. " + HELPER.date2string(guest2roomDTO.getDtoA().getDateOfBirth());
		if (guest2roomDTO.getMonthlyCosts() != null) {
			description += ",  " + HELPER.cent2euro(guest2roomDTO.getMonthlyCosts().value()) + " Euro (Monat)";
		}
		if (guest2roomDTO.getOwnCosts() != null && guest2roomDTO.getOwnCosts().value() > 0) {
			description += ",  " + HELPER.cent2euro(guest2roomDTO.getOwnCosts().value()) + " Euro (EB)";
		}
